...
namespace NewAgeWorship.Desktop
{
    public sealed class LocalAiAssistant
    {
        ...
        private const string SystemPrompt="You are NEWAGE WORSHIP's local operator assistant. Be concise, factual and operational. Never claim that you projected, changed Program, or performed an action. Suggestions remain private until a human approves them.";
        ...
        public async Task<string> CompleteAsync(string userText)
        {
            ...
            if(prompt.Length==0) return "Enter an operator request first. Program output was not changed.";
            var args="-m "+Q(_model)+" --system-prompt "+Q(SystemPrompt)+" --prompt "+Q(prompt)+" -n 96 --temp 0.2 --ctx-size 1024 --log-disable --no-display-prompt --no-show-timings --simple-io --single-turn";
            ...
            using(var p=new Process{StartInfo=psi})
            {
                ...
                if(p.ExitCode!=0)
                {
                    ...
                    return "Local AI failed safely: "+detail;
                }
